-- Fix room ID to allow custom 6-character codes instead of UUIDs

-- First, drop foreign key constraints that reference rooms.id
ALTER TABLE public.players DROP CONSTRAINT IF EXISTS players_room_id_fkey;
ALTER TABLE public.scenes DROP CONSTRAINT IF EXISTS scenes_room_id_fkey;
...
-- Recreate choices table (no changes needed)
...
-- Recreate votes table (no changes needed)
